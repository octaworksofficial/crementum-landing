  ...
  async function onSubmit(payload: EmailPayload) {
    try {
      const res = await fetch('/api/sendEmail', {
        ...
        body: JSON.stringify({ subject: 'CRMENTUM İletişim Formu', ...payload }),
      });
  ...
  return (
    <Wrapper>
      <Form onSubmit={handleSubmit(onSubmit)}>
        {hasErrored && <ErrorMessage>Email gönderilemedi. Lütfen tekrar deneyin.</ErrorMessage>}
        <InputGroup>
          <InputStack>
            {errors.name && <ErrorMessage>İsim gereklidir</ErrorMessage>}
            <Input placeholder="Adınız Soyadınız" id="name" disabled={isDisabled} {...register('name', { required: true })} />
          </InputStack>
          <InputStack>
            {errors.email && <ErrorMessage>Email gereklidir</ErrorMessage>}
